"""Render every r12 deliverable from the committed .blend, headless.

ONE COMMAND, from the repo root:

    blender --background src-assets/blender/SM_VoxelDwarf_Miner01.blend \
            --python src-assets/blender/render_r12.py -- <mode> [args]
...
Writes src-assets/renders/r12/. Authoring happens in Wolf's running Blender through the MCP
addon (round 12 sec.8); only this and export_dwarf.py are allowed to run headless, and this
script NEVER edits geometry -- it poses, renders, restores, and composites.

Adapted from render_r11.py (itself from render_r9.py, the script that produced the round
Wolf accepted on form); only the revision string and the reference-plane names changed.
Two things are new. STAGE RENDERS KEEP THE REFERENCE PLANES VISIBLE, because round 12 judges
...
def show_refs(on):
    ...
    for name in ("r12_ref_front", "r12_ref_side"):
        ob = bpy.data.objects.get(name)
        if ob is not None:
...
def final():
    ...
    arm, _ = bones()
    if arm is None:
        print("RENDER r12 -- unrigged figure, skipping the pose and deflection sheets")
        return written
...
def main():
    ...
    if mode == "stage":
        ...
        written = final()
    print("")
    print("RENDER r12 %s -- %d files" % (mode, len(written)))
    for path in written:
