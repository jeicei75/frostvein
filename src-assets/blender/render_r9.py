"""Render every r9 deliverable from the committed .blend, headless.

ONE COMMAND, from the repo root:

    blender --background src-assets/blender/SM_VoxelDwarf_Miner01.blend \
            --python src-assets/blender/render_r9.py

Writes src-assets/renders/r9/. Authoring happens in Wolf's running Blender through the MCP
addon (round 9 sec.8); only this and export_dwarf.py are allowed to run headless, and this
...
SCALE = {"head": 0.42}          # ortho width in metres; everything else frames the whole figure


def camera(view):
    name = "r9cam." + view.replace(".", "_")
...
def render(path, view="front", res=(640, 940), flat=False, ortho=None):
...
# ---------------------------------------------------------------- posing
def bones():
    arm = next(o for o in bpy.data.collections[COLL].objects if o.type == 'ARMATURE')
    return arm, arm.pose.bones
...
def main():
    ...
    # 1. the textured stage set, and the five final views in both lightings
    for view in ("front", "side", "quarter", "back"):
        written.append(render(f"stage-4-textured-{view}.png", view))
    ...
    # 2. beside the frames, matched scale: f088 is the depth authority, f104 the front one
    ...
    # 3. the side profile beside the orthographic sheet it was measured from. The crop spans
    ...
    # 4. readability: the figure at exactly 100 px and 60 px tall, blown up nearest-neighbour
    strips = []
    for px in (100, 60):
    ...
    # 5. the carry pose, and one deflection render per joint group
    ...
    written.append(render("pose-carry-front.png", "front"))
    written.append(render("pose-carry-quarter.png", "quarter"))
    written.append(render("pose-carry-side.png", "side"))
    reset()
    ...
    print("")
    print("RENDER r9 -- %d files" % len(written))
    for path in written:
