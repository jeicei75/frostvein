...
## Deferred from: Wolf's look review of 10.4's mesh trees (2026-09-02)

- **The tree ROOT reads artificial where the trunk meets the terrain.** Wolf's call on the
  corrected captures, and explicitly NOT 10.4's to fix. Two separable halves, and they belong to
  different kinds of story:
  - **Asset half — the root geometry itself.** The pines are generated by
    `10-2-signoff/voxel_pine.py`; the trunk simply ends at the mesh's base plane. There is no
    flare, no root spread, no irregularity where a real conifer meets ground. Fixing this means
    regenerating the GLBs, so it is an asset story in 10.2's shape, not a client change.
  - **Client half — nothing softens the junction.** `spawn_tree_mesh`
    (`crates/gui/src/project.rs:2205`) places the mesh at `world_to_render(base) - Vec3::Y * 0.5`,
    i.e. exactly on the base cell's floor plane, with no skirt geometry and no terrain blend. Worth
    knowing: the cube trees DID have a ground-level foliage ring and **9.4 removed it**, so the
