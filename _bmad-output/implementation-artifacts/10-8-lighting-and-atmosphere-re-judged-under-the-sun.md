...
### AC6's venue qualifier — added 2026-09-09, after the vehicle walk

**The re-calibrated capture ceilings are DEVPOD-VENUE constants.** `NEAR_WHITE_AREA_CEILING`
0.946072 % and the blown-pool 0.623806 % were derived from the approved frame, which was captured
headless under lavapipe. Measured on the delivery GPU at the same build and resolution, near-white
reads **1.0930 %** — an additive ~0.5 pp higher, confined to the bright tail. AC6 is still MET as
written (the guard discriminates on the venue it runs in, which is the gate), but any reading of it
as "the shipped look passes the guard" is true only in the devpod. Do not raise the ceiling to
absorb the venue gap — 10.4's standing ruling, and it would blind the gate by the same margin.

### §4 and §6 as answered at the seat — 2026-09-09

**AC12's six-word check is ANSWERED and closed:** Wolf, on the built branch, *"I am happy now"*.
Issue #75's *"lighting is still way off"* is **no longer true**, which is UX-DR22's question.

**NOT closed, and deliberately recorded as open:**
- **The hover slab (§4b) still has defects** — *"there are some issues with slabs still but not
  going to do anything for those now"*. That is DEFERRED BY WOLF, not closed. Do not record this
  eye-check as passed.
- **The three mark modes (§4c) — ANSWERED 2026-09-09: *"4c yes it's ok"*.** Passed at the seat.
  Wolf added *"need to recheck in another story it later on not now"*, so it is closed for 10.8
  with a recheck owed to a later story, not left open here.
- **§6, the k=1 flank look:** Wolf saw silvered walls and accepted them. Investigated rather than
  filed — they are the snow CELL's cube faces (`worldgen.rs:100`), not the cap AC14 fixed. See the
  Change Log and `deferred-work.md`. AC14 stands; this is terrain-story material.
...
- [~] **Task 6b — The flank rule and the atmosphere defects** (AC: 14, 15) — **AC14 DONE**,
  ...
  - [~] For each Ruling 3 defect: change the constant, file the before/after pair with figures,
        correct the pin that names it. **(a) fog and (d) rim are DEFERRED by Wolf.**
        **(c) flake size is RULED HALF and LANDED** (Ruling 7, gate green, pin re-pinned).
        **(b) the spawn band is NOT ACHIEVABLE by a constant** — flakes spawn in a fixed disc
        (radius 48, heights 11–31) around the camp, so any camera looking past it sees the field
        end. Wolf asked "possible from all view angles?"; the answer is no without anchoring the
...
| 2026-09-09 | **The vehicle's perf run closed a question that had been open since 2026-09-07, and exposed two instrument defects.** Wolf logged 8,711 frames over 61 s at `4738bd4` **with digging, zooming and rotating**. **`dirty_tiles` is CONFIRMED IN THE FIELD for the first time: 33 edit frames in the CSV against 33 `rebuilt … chunks` lines in the console — two independent instruments agreeing exactly.** Prior runs printed `edit frames — none recorded`, which is precisely what the BROKEN instrument printed, so this is the first evidence the fix works on hardware. **First real edit cost: p50 12.48 ms, p95 16.98, worst 18.34 ms**, zero hitches ≥2x median, over 38 changed tiles — and every rebuild touched **1 or 2 chunks** (`mesh_build_ms` 4–10) against 10.6's finding that one dug tile touched 121. **CAVEAT the summary's own headline needs:** it reports an edit frame at "1.80x the median steady frame", but steady p50 6.92 ms is 1000/144 = the VSYNC CAP, so the denominator is the panel and the true multiple is larger; the edit figures themselves are unclamped and real. The ~300 ms worst frame seen in all three runs (304 / 337.65 / 326.49) is the **window resize** recreating the swapchain, not a gameplay hitch. **Defect 1, filed as #84:** `--frames` is parsed, range-validated and then silently ignored without `--capture` (`ingest.rs:482-493`) — it cost an 8.5-minute run and a wrong line in this story's card. **Defect 2, FIXED in `bf8324c`:** `ingest.rs:708`'s promise that a headless and a vehicle frame are the same shape was enforced for headless ONLY; a windowed capture took the window's size, and since the window does not start maximised the operator resizes it every session. The range check now names its resolution and refuses any other shape, RED first, with the PNG still written before the refusal fires. 10.8's own vehicle capture was verified 1280x720, so the venue comparison recorded above stands. |
| 2026-09-09 | **§3 of the card WALKED ON THE VEHICLE: exit 101, near-white 1.0930 % against the 0.946072 % ceiling — and the ceiling, not the look, is what is wrong.** AC6 re-calibrated the ceilings on the approved frame and **the approved frame is a lavapipe frame**, so the constant is a DEVPOD-venue constant that cannot hold on the delivery GPU. Same build `c124615`, same 1280x720: RTX 4080 reads **1.0930 %** where lavapipe reads 0.4733 / 0.6047 %. **10.4 already measured this and wrote it down** (`10-4-signoff/README.md` § "The near-white ceiling reads WORSE on the GPU": 2.2071 % GPU vs 1.8159 % lavapipe) — two measurements, both an **additive ~0.4–0.6 pp GPU penalty**, ratios disagreeing (1.22x, 1.81x) so it is an offset and two points support no law. **The gap is confined to the bright tail:** mean 66.50 vs 64.99/65.30 and shade-band 55.51 % vs 55.44/55.26 % are nearly identical, while p99 goes 180 → 206. Real specular and emissive highlights are what lavapipe is not reproducing. **On the venue that matters the treatment HALVED near-white, 2.2071 % → 1.0930 %.** Ceiling NOT raised: 10.4's standing ruling forbids absorbing a venue gap, and ~0.5 pp of headroom would blind the gate's own regression check by the same amount. The guard runs in the devpod, so a devpod ceiling is right for its job. **The card's exit-0 prediction was wrong, in the same way 10.4's card was wrong, from the same venue assumption** — corrected in place, along with a real usability defect: the §3 capture run panics and takes the process with it, so §4–§6 need a relaunch without `--capture`, which the card had not said. Vehicle shot committed by Wolf as `10-8-signoff/10-8-vehicle.png` (`3753d6e`). **No vehicle noise floor exists** — one GPU capture at this build; a second run is the cheapest thing left. §4–§6 (AC12's eye-checks, AC15's (b) and (c), the k=1 flank look) are still UNANSWERED. |
...
| `_bmad-output/implementation-artifacts/10-8-signoff/10-8-vehicle.png` | NEW (Wolf, `3753d6e`), the first GPU capture of the shipped look |
| `crates/gui/src/atmosphere.rs` | Ruling 7: `snowflake_scale` to half, its pin re-pinned to the ruled value |
| `_bmad-output/implementation-artifacts/10-8-signoff/{flakes.py,AC15c-flake-size.md,AC15c-flake-*.png}` | NEW, the ladder, its instrument and its frames |
| `docs/tech-art-guidelines.md` | ruled flake size, with the lavapipe caveat |
| `crates/gui/src/capture.rs` | `CAPTURE_SIZE` contract, `assert_calibrated_frame_shape`, resolution in the range-check line, two tests |
| `crates/gui/src/ingest.rs` | `HEADLESS_SIZE` derives from `capture::CAPTURE_SIZE` instead of restating it |
| `_bmad-output/implementation-artifacts/mutations/9-1-the-frame-stops-blowing-out.sh` | both rows re-pinned to the new report line |
