...
| 2026-08-03 | Hardened the observability instrument before starting 2.3: `--frames` now warns on stderr when `NO_COLOR` suppresses colour, so a capture can no longer be silently vacuous, and two tests drive the real binary end to end (walk colour reaches the capture; the warning fires). Mutation set grown to 12, all killed. |
