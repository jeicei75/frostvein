...
Run with Blender: ``blender --background --python candidate_bench.py -- SNAPSHOT OUT [TAPER]``.
"""
...
def main():
    global FOLIAGE_TAPER
    if bpy is None:
        raise SystemExit("run this script with blender")
    args = sys.argv[sys.argv.index("--") + 1 :] if "--" in sys.argv else []
    if len(args) not in (2, 3):
        raise SystemExit("usage: candidate_bench.py -- <snapshot.json> <out.png> [taper]")
    try:
        if len(args) == 3:
            FOLIAGE_TAPER = tuple(float(value) for value in args.pop().split(","))
        render(args)
