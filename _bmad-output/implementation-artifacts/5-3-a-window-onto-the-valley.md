...
- **STANDING TOOL DEFECT, surfaced for Wolf's ruling and deliberately NOT fixed in this story** (it maps to no task in 5.3, and `scripts/mutate.sh` is *not* a forge-process FILE, so a local fix has no propagation cost): `mutate.sh:61` treats **any** non-zero `cargo test` exit as KILLED, which cannot distinguish "a test caught the sabotage" from "the sabotage did not compile". A non-compiling mutation proves nothing and should be reported as a survivor. This is the **second** false-green of the same class in this one script — its own header records the first (an empty mutations file printing `All mutations killed.`, found at 3.3's review by feeding it a truncated file). Ten tables and ~50 mutations exist across the project and any of them may carry vacuous kills; this one was invisible for two days and through three Codex self-gate passes because the tool prints no assertion for such a kill and nobody reads a KILLED row's body. Detection is cheap once looked for: **a KILLED row with no `panicked at` line under it**.
...
| 2026-08-13 | Orchestrator re-verification, independent of the dev run: gate GREEN 63.16 s warm, all twelve files present, all commits `Völundr` and per-task, display absence re-measured. **AC27 was falsely green** — the `exposed terrain returns every solid tile` mutation killed by SYNTAX ERROR (`expected ';', found 'NEIGHBOURS'`), and `mutate.sh` counts any non-zero cargo exit as KILLED, so the table claimed five kills while pinning four. Rewritten to remove the neighbour scan rather than the guard; it now dies on `assertion failed: !is_exposed(&enclosed, [1, 1, 1])`. Coverage was checked separately and was already sound, so no test was added or changed. The underlying `mutate.sh` defect is recorded for Wolf's ruling, not fixed here. Story stays **in-progress**: Tasks 0 and 8 need a display and are blocked on the `rebelspice` bring-up. |
